			...
			if(tag.getBoolean("isCasting") && entity instanceof EntityPlayer)
			{
				...
				if(tag.getInteger("castTime") >= maxCastTime)
				{
					...
					if(dimension != oldDimension)
					{
						...
						worldServer.getMinecraftServer().getPlayerList().transferPlayerToDimension(entityPlayerMP, dimension, new HearthstoneTeleporter(worldServer, bedX, bedY, bedZ));
		...
		if(!world.isRemote)
		{
			...
			if(player.isSneaking())
			{
				...
				if(world.getBlockState(blockPos).getBlock().isBed(state, world, blockPos, player))
				{
					...
					tagCompound.setInteger("bedDimension", player.getEntityWorld().provider.getDimension());
