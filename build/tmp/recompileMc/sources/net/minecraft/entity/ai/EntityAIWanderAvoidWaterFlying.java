...
public class EntityAIWanderAvoidWaterFlying extends EntityAIWanderAvoidWater
{
    ...
    private Vec3d func_192385_j()
    {
        ...
        Iterable<BlockPos.MutableBlockPos> iterable = BlockPos.MutableBlockPos.func_191531_b(MathHelper.floor_double(this.entity.posX - 3.0D), MathHelper.floor_double(this.entity.posY - 6.0D), MathHelper.floor_double(this.entity.posZ - 3.0D), MathHelper.floor_double(this.entity.posX + 3.0D), MathHelper.floor_double(this.entity.posY + 6.0D), MathHelper.floor_double(this.entity.posZ + 3.0D));
        ...
        while (true)
        {
            ...
            {
                Block block = this.entity.worldObj.getBlockState(blockpos$mutableblockpos1.setPos(blockpos1).move(EnumFacing.DOWN)).getBlock();
                boolean flag = block instanceof BlockLeaves || block == Blocks.LOG || block == Blocks.LOG2;

                if (flag && this.entity.worldObj.isAirBlock(blockpos1) && this.entity.worldObj.isAirBlock(blockpos$mutableblockpos.setPos(blockpos1).move(EnumFacing.UP)))
                {
                    break;
                }
            }
