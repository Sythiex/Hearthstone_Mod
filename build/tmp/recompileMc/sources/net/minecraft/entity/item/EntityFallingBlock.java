    ...
    public void onUpdate()
    {
        ...
        else
        {
            ...
            if (!this.worldObj.isRemote)
            {
                ...
                boolean flag1 = flag && this.worldObj.getBlockState(blockpos1).getMaterial() == Material.WATER;
                double d0 = this.motionX * this.motionX + this.motionY * this.motionY + this.motionZ * this.motionZ;

                if (flag && d0 > 1.0D)
                {
                    RayTraceResult raytraceresult = this.worldObj.rayTraceBlocks(new Vec3d(this.prevPosX, this.prevPosY, this.prevPosZ), new Vec3d(this.posX, this.posY, this.posZ), true);

                    if (raytraceresult != null && this.worldObj.getBlockState(raytraceresult.getBlockPos()).getMaterial() == Material.WATER)
                    {
                ...
                {
                    if (this.fallTime > 100 && !this.worldObj.isRemote && (blockpos1.getY() < 1 || blockpos1.getY() > 256) || this.fallTime > 600)
                    {
                ...
                else
                {
                    ...
                    if (!flag1 && BlockFalling.canFallThrough(this.worldObj.getBlockState(new BlockPos(this.posX, this.posY - 0.009999999776482582D, this.posZ))))
                    {
                        this.onGround = false;
                        return;
                    }
                    ...
                    if (iblockstate.getBlock() != Blocks.PISTON_EXTENSION)
                    {
                        ...
                        {
                            if (this.worldObj.func_190527_a(block, blockpos1, true, EnumFacing.UP, (Entity)null) && (flag1 || !BlockFalling.canFallThrough(this.worldObj.getBlockState(blockpos1.down()))) && this.worldObj.setBlockState(blockpos1, this.fallTile, 3))
                            {
                                if (block instanceof BlockFalling)
                                {
                                    ((BlockFalling)block).onEndFalling(this.worldObj, blockpos1, this.fallTile, iblockstate);
                                }
